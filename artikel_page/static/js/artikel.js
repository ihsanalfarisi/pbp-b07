$(document).ready(function(){
    ...
    function MainPage() {
      ...
      $.ajax({
        url: "http://127.0.0.1:8000/artikel/json", 
        dataType: 'json',
        success: function(hasil){
          if (hasil.length == 0) {
            ...
            <br>
            <p class="text-muted fst-italic fs-1">
              Artikel Belum Tersedia
            </p>
            `)
          } else {
            var latest = hasil.length-1
            var tanggal = hasil[latest].fields.time_published.slice(0,10)
            $("#article").append(`
            <div class="main-article p-4 p-md-5 mb-4 text-white rounded bg-dark">
              <div class="col-md-6 px-0">
                ...
                <p class="lead mb-0"><a href="#" id="${hasil[latest].pk}" class="detail-artikel text-white fw-bold">Continue reading..</a></p>
              </div>
            </div>
            `);
            $("#article").append('<div class="other-article row mb-2">')
            for(i=hasil.length-2; i >= 0;i--){
              var tanggal = hasil[i].fields.time_published.slice(0,10)
              ...
              <div class="col-md-6">
                <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                  <div class="col p-4 d-flex flex-column position-static">
                    ...
                    <a href="#" id="${hasil[i].pk}" class="detail-artikel stretched-link">Continue reading..</a>
                  </div>
    ...
    $('.article-page').on("click",'.detail-artikel',function(){
        var id = $(this).attr('id');
          $.ajax({
          url: 'http://127.0.0.1:8000/artikel/details/'+id, 
          dataType: 'json',
          success: function(hasil){
            var tanggal = hasil[0].fields.time_published.slice(0,10)
            $(".article-page").empty()
            $(".article-page").append(`
